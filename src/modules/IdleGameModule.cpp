...
/**
 * Zwraca true z prawdopodobieństwem podanym w procentach, np.
 * checkChance(50.0) -> ~50% szans na true
 * checkChance(1.0)  -> ~1% szans na true
 */
bool checkChance(float chancePercent)
{
    // Zabezpieczenia, gdyby ktoś podał wartości spoza [0..100]
    ...
    // Funkcja random(n) w Arduino zwraca liczbę całkowitą z zakresu [0..n-1].
    // Tutaj chcemy przedział [0..99].
    int r = random(100); 

    // Jeśli wylosowana wartość jest mniejsza niż szansa, to "trafiliśmy"
    // np. random(100) = 37 i chancePercent = 50.0 -> 37 < 50 -> true
