...
void DtnOverlayModule::handleData(const meshtastic_MeshPacket &mp, const meshtastic_FwplusDtnData &d)
{
    ...
    if (it == pendingById.end()) {
        ...
        //fw+
        if (isReceiptCustody && d.custody_path_count > 0) {
            ...
            if (occurrences > 1) {
                LOG_INFO("DTN: Receipt loop prevention - rejecting receipt custody id=0x%x (path already contains us %d×, first pos=%d)",
                         d.orig_id, occurrences, firstIdx);
                return; // Multiple occurrences indicate we processed this receipt already
            }
        }
                                    ...
                                    uint8_t custodyPathCount, const uint8_t *custodyPath)
{
    ...
    if (useNativeUnicast) {
        ...
        // Receipt fallback to native DM breaks protobuf parsing (shows as CLEAR_TEXT).
        // Keep fallback enabled only for PROGRESSED hints; final statuses must stay DTN-only.
        bool allowFallback = (status == meshtastic_FwplusDtnStatus_FWPLUS_DTN_STATUS_PROGRESSED);
