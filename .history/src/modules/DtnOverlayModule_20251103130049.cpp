    ...
    if (it == pendingById.end()) {
        ...
        if (isReceiptCustody && d.custody_path_count > 0) {
            ...
            if (occurrences > 0) {
                ...
                if (!isTail || occurrences > 1) {
                    LOG_INFO("DTN: Receipt loop prevention - rejecting receipt custody id=0x%x (path contains us %d×, first pos=%d, tail=%d)",
                             d.orig_id, occurrences, firstIdx, (int)d.custody_path_count - 1);
                    return; // Reject custody to prevent loop; legitimate tail continues
                }
            }
        }
