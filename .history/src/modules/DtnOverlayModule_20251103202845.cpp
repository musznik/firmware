...
void DtnOverlayModule::tryForward(uint32_t id, Pending &p)
{
    ...
    if (p.isReceiptCustody && target != 0 && target != p.data.orig_to) {
        mp->hop_limit = 1; // Receipt relay hop should stay local to forced edge
    }
                                    ...
                                    uint8_t custodyPathCount, const uint8_t *custodyPath)
{
    ...
    if (useNativeUnicast) {
        ...
        bool isFinalStatus = (status == meshtastic_FwplusDtnStatus_FWPLUS_DTN_STATUS_DELIVERED ||
                              status == meshtastic_FwplusDtnStatus_FWPLUS_DTN_STATUS_FAILED ||
                              status == meshtastic_FwplusDtnStatus_FWPLUS_DTN_STATUS_EXPIRED);
        if (isFinalStatus) {
            const uint32_t kFinalReceiptTtlMs = 20000; // keep final receipts briefly for retries
            if (receiptPending.data.ttl_remaining_ms == 0 || receiptPending.data.ttl_remaining_ms > kFinalReceiptTtlMs) {
                receiptPending.data.ttl_remaining_ms = kFinalReceiptTtlMs;
            }
            if (receiptPending.receiptMsg.ttl_remaining_ms == 0 || receiptPending.receiptMsg.ttl_remaining_ms > kFinalReceiptTtlMs) {
                receiptPending.receiptMsg.ttl_remaining_ms = kFinalReceiptTtlMs;
            }
        }
