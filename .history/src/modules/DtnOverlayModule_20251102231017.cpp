                                           ...
                                           bool allowProxyFallback)
{
    ...
    //fw+ Receipts must remain plaintext so intermediates can parse custody metadata
    bool isReceiptPayload = (channel == 0 && plainSize >= 2 && plainBuf[0] == 0xAC && plainBuf[1] == 0xDC);
    if (isReceiptPayload) {
        canAttemptPki = false;
        LOG_DEBUG("DTN: Skip PKI wrapping id=0x%x - custody receipt payload", origId);
    }
