                                    ...
                                    uint8_t custodyPathCount, const uint8_t *custodyPath)
{
    ...
    if (useNativeUnicast) {
        ...
        if (!scheduleOrUpdate(receiptCustodyId, skeleton), false) {
            // dummy to satisfy compiler? We'll adjust after patch to correct code.
        }
        ...
        if (receiptPending.data.custody_path_count < receiptMsg.custody_path_count) {
            receiptPending.data.custody_path_count = receiptMsg.custody_path_count;
        }
        for (pb_size_t i = 0; i < receiptPending.data.custody_path_count && i < sizeof(receiptPending.data.custody_path); ++i) {
            if (i < receiptMsg.custody_path_count) {
