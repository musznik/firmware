...
void DtnOverlayModule::deliverLocal(const meshtastic_FwplusDtnData &d)
{
    ...
    auto sendPlaceholder = [&](const char *msg, const char *tag) {
        sendDecodedToPhone(reinterpret_cast<const uint8_t *>(msg), strlen(msg), tag);
    };
    ...
    if (d.use_pki_encryption) {
        ...
        if (!srcInfo || srcInfo->user.public_key.size != 32) {
            LOG_ERROR("DTN: Missing source public key for PKI payload id=0x%x from=0x%x", d.orig_id, d.orig_from);
            sendPlaceholder("[Encrypted message - missing public key]", "PKI-MISSING-KEY");
            return;
        }

        if (d.payload.size <= MESHTASTIC_PKC_OVERHEAD) {
            LOG_ERROR("DTN: PKI payload too small id=0x%x size=%u", d.orig_id, (unsigned)d.payload.size);
            sendPlaceholder("[Encrypted message - invalid payload]", "PKI-INVALID");
            return;
        }
        ...
        if (!decrypted) {
            LOG_ERROR("DTN: PKI decrypt failed id=0x%x from=0x%x", d.orig_id, d.orig_from);
            memset(plaintext, 0, sizeof(plaintext));
            sendPlaceholder("[Encrypted message - decrypt failed]", "PKI-DECRYPT-FAIL");
            return;
        }
                                           ...
                                           bool allowProxyFallback)
{
    ...
    if (configEnableE2ePki && origFrom == nodeDB->getNodeNum()) {
