...
void DtnOverlayModule::tryForward(uint32_t id, Pending &p)
{
    ...
    if (p.isReceiptCustody && target != 0 && target != p.data.orig_to) {
        uint8_t hopsToTarget = getHopsAway(target);
        uint8_t constrainedHopLimit = 1;
        if (hopsToTarget == 255) {
            constrainedHopLimit = 3; // unknown distance – allow short flood but avoid mesh-wide broadcast
        } else if (hopsToTarget >= 1) {
            uint8_t candidate = (uint8_t)(hopsToTarget + 1); // one extra hop to clear edge boundary
            if (candidate > 3) candidate = 3;
            constrainedHopLimit = candidate;
        }
        mp->hop_limit = constrainedHopLimit;
    }
                                    ...
                                    uint8_t custodyPathCount, const uint8_t *custodyPath)
{
    ...
    if (useNativeUnicast) {
        ...
        if (isFinalStatus) {
            const uint32_t kFinalReceiptTtlMs = 20000;
            if (receiptMsg.ttl_remaining_ms == 0 || receiptMsg.ttl_remaining_ms > kFinalReceiptTtlMs) {
                receiptMsg.ttl_remaining_ms = kFinalReceiptTtlMs;
            }
        }
